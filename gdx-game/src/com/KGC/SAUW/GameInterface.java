...
public class GameInterface {
	...
	public void initilizate(Crafting c, final ModAPI ModAPI, final MainGame game, final Langs langs, final World w) {
					...
					for (int i = currentTab * 30; i < crafting.crafts.size(); i++) {
						...
						if (currentCraft == i) {
							for (int j = 0; j < crafting.crafts.get(i).ingr.length; j++) {
							    ...
							    if (j == 0) c0.IC.drawMultiLine(batch, pl.getCountOfItems(crafting.crafts.get(i).ingr[j][0]) + "/" + crafting.crafts.get(i).ingr[j][1], c0.X, c0.Y + c0.IC.getCapHeight(), c0.width, BitmapFont.HAlignment.RIGHT);
								if (j == 1) c1.IC.drawMultiLine(batch, pl.getCountOfItems(crafting.crafts.get(i).ingr[j][0]) + "/" + crafting.crafts.get(i).ingr[j][1], c1.X, c1.Y + c1.IC.getCapHeight(), c1.width, BitmapFont.HAlignment.RIGHT);
								if (j == 2) c2.IC.drawMultiLine(batch, pl.getCountOfItems(crafting.crafts.get(i).ingr[j][0]) + "/" + crafting.crafts.get(i).ingr[j][1], c2.X, c2.Y + c2.IC.getCapHeight(), c2.width, BitmapFont.HAlignment.RIGHT);
								if (j == 3) c3.IC.drawMultiLine(batch, pl.getCountOfItems(crafting.crafts.get(i).ingr[j][0]) + "/" + crafting.crafts.get(i).ingr[j][1], c3.X, c3.Y + c3.IC.getCapHeight(), c3.width, BitmapFont.HAlignment.RIGHT);
								if (j == 4) c4.IC.drawMultiLine(batch, pl.getCountOfItems(crafting.crafts.get(i).ingr[j][0]) + "/" + crafting.crafts.get(i).ingr[j][1], c4.X, c4.Y + c4.IC.getCapHeight(), c4.width, BitmapFont.HAlignment.RIGHT);
								if (j == 5) c5.IC.drawMultiLine(batch, pl.getCountOfItems(crafting.crafts.get(i).ingr[j][0]) + "/" + crafting.crafts.get(i).ingr[j][1], c5.X, c5.Y + c5.IC.getCapHeight(), c5.width, BitmapFont.HAlignment.RIGHT);
							}
		...
		//"{\"standart\":{\"header\":{\"text\":{\"text\":\"\"}}},\"isBlockInterface\":false, \"inventory\" : {\"standart\":false}, \"background\" : {\"standart\" : false, \"full\" : false}}, \"elements\" : {}, \"drawing\" : []}"
	...
	public void render(Player pl, String debug) {
		...
		if (!isInterfaceOpen) this.debug.drawMultiLine(batch, debug, 3 + interfaceCamera.X, interfaceCamera.H - WIDTH / 16 + interfaceCamera.Y);
		inv.render(pl);
